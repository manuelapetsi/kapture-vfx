...
ALLOWED_IMAGE_HEADERS = ("data:image/jpeg", "data:image/png", "data:image/webp")
...
class FrameProcessor:
    ...
    def clear_background(self) -> None:
        self.cloak.background_frame = None
        self.background_ready = False
    ...
    def set_params(
        ...
        preview_mask: Optional[bool] = None,
        keep_largest: Optional[bool] = None,
        min_area_ratio: Optional[float] = None,
        skin_protect: Optional[bool] = None,
    ) -> None:
        self.cloak.set_params(
            blur_ksize=blur_ksize,
            morph_iterations=morph_iterations,
            morph_kernel_size=morph_kernel_size,
        )
        self.cloak.set_filters(
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        if self.preview_mask:
            ...
            return cv2.addWeighted(frame_bgr, 0.6, overlay, 0.4, 0)
    ...
    def decode_base64_image(data_uri: str) -> Optional[np.ndarray]:
        ...
        header = header.lower()
        if not any(header.startswith(prefix) for prefix in ALLOWED_IMAGE_HEADERS):
            return None

        try:
            binary = base64.b64decode(b64data, validate=True)
            if len(binary) > MAX_DECODED_IMAGE_BYTES:
                return None
            ...
            if height > 4000 or width > 4000 or height < 10 or width < 10:
                return None

            return frame
        except (base64.binascii.Error, ValueError, cv2.error):
            return None
        except Exception:
            return None
